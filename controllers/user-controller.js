...
module.exports = new class {
  exist = {
    ...
    controller: async (req, res) => {
      ...
      for (const user of await UserModel.all()) {
        if (email !== undefined) {
  ...
  update = {
    ...
    controller: async (req, res) => {
      ...
      UserModel.find(sessionUser.id)
        .then(user => {
          user.update(sessionUser.email, sessionUser.password, inputNickname, filePath);
          user.save();
        }).then(() => {
        res.json({
  ...
  password = {
    ...
    controller: (req, res) => {
      ...
      UserModel.find(sessionUser.id).then(
        user => {
